...
const fetchGroupWiseExpenseSummary = async (
  ...
  const thisMonthExpenses = (expenses: ExpenseType[]) => {
  ...
  const lastMonthExpenses = (expenses: ExpenseType[]) => {
    return expenses.filter((expense) => {
      const expenseDate = new Date(expense.createdAt);
  ...
  const calculateShare = (filteredExpenses: ExpenseType[]) => {
    return filteredExpenses.reduce((totalShare, expense) => {
      const userShare =
        expense.participants.find(async (participant) => {
          const participantUser = await User.findById(participant.participant);
          if (participantUser?._id == userId) return true;
          return false;
        })?.share || 0;
